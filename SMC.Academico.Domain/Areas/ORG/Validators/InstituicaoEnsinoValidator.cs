...
{
    public class InstituicaoEnsinoValidator : SMCValidator<InstituicaoEnsino>
    {
        /// <summary>
        /// Validações:
        /// •	Campos Sigla e Logotipo são de preenchimento obrigatório.
        /// •	Pelo menos um endereço obrigatório - Tipo Correspondência.
        ...
        protected override void DoValidate(InstituicaoEnsino item, SMCValidationResults validationResults)
        {
            ...
            if (item.Enderecos.SMCCount() > 1 &&
                item.Enderecos.GroupBy(t => t.TipoEndereco).Any(g => g.Count() > 1))
            {
                this.AddPropertyError(x => x.Enderecos, String.Format(MessagesResource.MSG_EnderecoDuplicado, MessagesResource.TIPO_ENTIDADE_InstituicaoEnsino));
            }

            // Verifica se informou obrigatoriamente 1 endereço de correspondencia
            var endCorrespondencia = item.Enderecos.SMCCount(f => f.Correspondencia.HasValue && f.Correspondencia.Value);
            ...
            if (item.Telefones.SMCCount() > 1 &&
                item.Telefones.GroupBy(t => t.TipoTelefone).Any(g => g.Count() > 1))
            {
                this.AddPropertyError(x => x.Telefones, String.Format(MessagesResource.MSG_TelefoneDuplicado, MessagesResource.TIPO_ENTIDADE_InstituicaoEnsino));
            }

            // Verifica se informou um telefone comercial
            if (item.Telefones.SMCCount() == 0 ||
            ...
            if (item.EnderecosEletronicos.SMCCount() > 1 &&
                item.EnderecosEletronicos.GroupBy(e => e.TipoEnderecoEletronico).Any(g => g.Count() > 1))
            {
                this.AddPropertyError(x => x.EnderecosEletronicos, String.Format(MessagesResource.MSG_EnderecoEletronicoDuplicado, MessagesResource.TIPO_ENTIDADE_InstituicaoEnsino));
            }

            // Verifica se infomrou o email e webmail
            if (item.EnderecosEletronicos.SMCCount() < 2 ||
            ...
            if (item.EnderecosEletronicos.SMCCount() > 0)
            {
        ...
        /// - Tamanho máximo do Nome reduzido = 50
        /// - Tamanho máximo do Nome Complementar = 100
        /// - Tamanho máximo da Sigla = 15
        /// </summary>
        public override void Configure()
        {
            this.Property(p => p.Nome).HasMaxLength(100);
            this.Property(p => p.NomeReduzido).HasMaxLength(50);
            this.Property(p => p.NomeComplementar).HasMaxLength(100);
